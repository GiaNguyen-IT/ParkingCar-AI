...
import cv2
import numpy as np
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QFileDialog, QSlider, QStyle, QMessageBox
...
class ParkingLotApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.save_button = QPushButton('Save Shapes', self)
    ...
    def save_shapes(self):
        file_name, _ = QFileDialog.getSaveFileName(self, 'Save Shape File', '', 'Text Files (*.txt)')
        if file_name:
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self.scaled_pixmap:
            ...
            x = event.x() - self.image_label.geometry().x() - x_offset
            y = event.y() - self.image_label.geometry().y() - y_offset

            if 0 <= x < pixmap_width and 0 <= y < pixmap_height:
                original_height, original_width, _ = self.frame.shape
                x = x * original_width / pixmap_width
                y = y * original_height / pixmap_height
